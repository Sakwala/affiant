...
# else -- a detail that has outlived the defect it describes is worse than no detail, because it
...
NOTES = (
    ...
    "restated here. "
    "(1) WHAT THIS RUN IS. It is a run of the tree at the commit the run log names, which is on "
    "the branch that moves this repository's pin to the rulebook's v0.2.0 and builds "
    "1.0.0-beta.3.1. That version HAS shipped -- ten NuGet packages, 2026-09-09 -- and the tree "
    "has taken changes since the tag, so this is a reading of that tree rather than a re-reading "
    "of the released artefacts. The five fixtures the rulebook's negative "
    "oracle lists against the SHIPPED "
