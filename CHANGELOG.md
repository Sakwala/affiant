...
10. **A tool result's discriminator is now `kind`.** *Client migration:* a TypeScript client that
...
11. **A provenance tag's `evidence` is now `note` on the wire.** *Client migration:* a client
...
12. **A tag carries `at` and `binding`, and an Affidavit carries `populatedConfidence` and
    `emptyFieldCount`.** Additive: a client that ignores unknown properties needs no change. A client
    that rejects them does.
13. **An instant is spelled `...T00:05:00.000Z`.** *Client migration:* none for anything that parses
...
14. **A `ReviewStatus` on the wire is lowercase.** *Client migration:* a client comparing against
    `"Pending"` compares against `"pending"`.
15. **`EvidenceCardRequest` gains seven optional constructor parameters** after `PriorAmendments`.
...
16. **`ProvenanceTag` gains a sixth positional parameter (`At`, defaulted `null`).** Existing
