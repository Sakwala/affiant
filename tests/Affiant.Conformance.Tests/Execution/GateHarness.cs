...
/// </para>
/// <para>
/// <b>There is no clock seam.</b> <c>ReviewGate</c> reads <c>DateTimeOffset.UtcNow</c> at four
/// sites and <c>DocketExpiryService</c> at a fifth; none is injectable. The fixture's clock is
/// therefore held here and used for every instant under the driver's own control, and where the
/// framework reads the wall clock instead, the fixture that depends on it fails with a diff saying
/// so. That gap is reported once, plainly, rather than being simulated away.
/// </para>
/// </remarks>
internal sealed class GateHarness : IDisposable
{
    ...
    public static GateHarness Build(GateSpec gate, DateTimeOffset clock)
    {
        ...
        {
            services.AddSingleton<IApprovalPolicy>(new FixturePolicy(policy));
        }
