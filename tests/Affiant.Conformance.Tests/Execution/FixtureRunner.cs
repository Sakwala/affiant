...
internal sealed class FixtureRunner
{
        ...
        CancellationToken ct)
    {
        ...
        observation["store"] = new JsonObject
        {
            ...
            // No row carries an execution outcome, so every approved row is an unexecuted one.
        ...
        if (under.EntryId is { } entryId && rows.FirstOrDefault(r => r.EntryId == entryId) is { } entry)
        {
            ...
            observation["entry"] = Observation.Entry(
                entry,
                new Observation.EntryFacts(executor.RequirementOf(entryId), parent?.EntryId));

            if (parent is not null)
            {
                observation["superseded"] = Observation.Entry(
                    parent,
                    new Observation.EntryFacts(executor.RequirementOf(parent.EntryId), null));
            }
        }
        ...
        // helper produces. This release exports none, so there is nothing to read and the key stays
        // absent — a fixture stating it fails, with (absent) on the actual side.
        _ = fixture;
        return observation;
    }
}
