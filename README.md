...
| `AddAffiantPolicies()` | Recommended, not throw-enforced | Skipped: every write falls through to the built-in `ReviewerConfirmation` default policy — a working, if unopinionated, fallback, not a startup failure. One thing inside it *is* enforced: a Standing Order that declares a `RiskThreshold` throws when the container builds it unless `SetRiskScoreCalculator<T>()` supplied a calculator |
