...
///
/// <para>
/// <b>Where this type belongs, and why it is here for now.</b> The marker's home is the Docket row —
/// the entry is what is blocked, and the card only reports it — and the change that gives
/// <c>DocketEntry</c> its <c>Blocked</c> column, the guarded transitions that refuse decisions on a
/// blocked row, and the pipeline paths that file one, is a separate change that this one is not
/// stacked on. Declaring the shape here lets the card envelope carry it now, typed and validated
/// against <c>schemas/0.1.0/blocked.schema.json</c>, instead of leaving a hole the row change would
/// have to fill twice. When the row lands, the row's marker is this type: nothing about the wire
/// shape changes, and a card built from a blocked row carries the row's own value.
/// </para>
///
/// <para>
